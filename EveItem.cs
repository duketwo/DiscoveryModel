...
namespace EveModel
{
	public class EveItem : EveInvType
	{
		public EveItem(EveObject obj)
			: base(obj)
		{
			if(this.IsValid && this.HasAttrString("typeID") )
		...
		public bool Sell(){
			if(Frame.Client.Session.InStation && Frame.Client.Session.StationId != 0 && Frame.Client.Session.StationId !=  null && Frame.Client.Session.StationId != -1){
				Frame.Log("[EveItem-Sell] Station id: " + Frame.Client.Session.StationId + " Locationid: " + this.LocationId  +" Typeid: "  + this.TypeId + " Itemid: " + this.ItemId + " IntItemID: " + (int)this.ItemId + " quantity: " + this.Quantity);
				Frame.Client.GetService("marketutils").CallMethod("Sell", new object[] { this.TypeId, this, 1 },true);
				return true;
			} else {
				Frame.Log("[EveItem-Sell] You can only sell in a station");
				return false;
			}
		}
		...
		public bool InjectSkill(){
			...
			if(Frame.Client.Const["categorySkill"].GetValueAs<int>() != this.CategoryId || !this.IsValid || this.ItemId == 0L){
				return false;
			}
			
			if(Frame.Client.GodmaService.CallMethod("CheckSkillRequirementsForType", new object[] { this.TypeId }, false).GetValueAs<bool>() == false){
				Frame.Log("[InjectSkill] CheckSkillRequirementsForType == false for this type, not  injecting");
				return false;
			}
				
			List<EveObject> eveObjects = new List<EveObject>();
			eveObjects.Add(new EveObject(this.PointerToObject,"",false));
			Frame.Client.menuSvcExtras["invItemFunctions"].CallMethod("InjectSkillIntoBrain", new object[] {  eveObjects  },true);
			return true;
		}
	}
}
